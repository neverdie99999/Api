                        ...
                        if '0300' in dataS.hex()[0:4] :
                            ...
                            if  botcomendenable==True:
                                ...
                                if '1200' in dataS.hex()[0:4] and '3f6c766c' in dataS.hex()[0:900] and Fox==True :
                                    client.send(bytes.fromhex(gen_msgv2(dataS.hex() ,"[00FF00][b][c]عذرا هده الميزة غير متوفرة بنسخة المجانية !!")))
                                    client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[00FF00][b][c]عذرا هده النسخة غير متوفرة بنسخة المجانية  !! "))))
                                    self.spam_foxy=False
